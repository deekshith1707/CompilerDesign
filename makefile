CC = g++
CFLAGS = -Wall -I$(SRC_DIR) -std=c++11
LEX = flex
LEXFLAGS = 
YACC = bison
YACCFLAGS = -d -v
...
# Source files for the refactored modules (now C++)
CPP_SOURCES = $(SRC_DIR)/main.cpp $(SRC_DIR)/ast.cpp $(SRC_DIR)/symbol_table.cpp $(SRC_DIR)/ir_context.cpp $(SRC_DIR)/ir_generator.cpp
...
# Object files for the refactored modules (now C++)
CPP_OBJECTS = $(OBJ_DIR)/main.o $(OBJ_DIR)/ast.o $(OBJ_DIR)/symbol_table.o $(OBJ_DIR)/ir_context.o $(OBJ_DIR)/ir_generator.o
...
# Compilation rules for the refactored modules (now C++)
